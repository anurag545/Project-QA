...
<span itemprop="name"><h2><a href="addsubcat.php">Add SubCategory</a></h2></span></div>
...
if(isset($_POST["submit"]))
{
  ...
  if($err==0)
  {
   ...
   $conn=mysqli_connect(host,uname,pass,dbname) or die ("Error in connection". mysqli_connect_error());
	$q="insert into addsubcat(CatID,SubCategoryName,SubCategoryPic) values ('$cid','$scname','$scpic')";
...
function xyz()
{
		if (document.form1.catid.selectedIndex== 0)
	{
 		alert('please choose category first');
		return false;
	}
	if(document.form1.subcatname.value.length<1)
	{
		alert("plz fill subcategory name");
		return false;
		}}

</script>
<form name="form1"  id="subcatform" class="form_modal_style" method="post" enctype="multipart/form-data" onSubmit="return xyz()">
...
<input type="text" class="name_user" name="subcatname" id="username"/>
<label for="subcategorypic">
SubCategory Pic</label>
<input type="file" name="subcatpic" id="catpic" />
<b><font color="#FF0000">
<?php
if(isset($msg))
print $msg;
?></font></b>
<input type="submit" name="submit" value="Add Sub Category" class="btn-submit">
</form>
</div>
