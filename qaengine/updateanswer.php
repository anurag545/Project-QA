...
<span itemprop="name"><h2><a href="updatequestion.php">List Of Questions</a></h2></span></div>
...
<form id="form1" name="form1" method="post" action="">
  <table width="400" cellspacing="0" cellpadding="5">
    <tr>
      <td><h4><font color="#000000">Choose Category</font></h4></td>
    ...
    <tr>
      <td><h4><font color="#000000">Choose SubCategory</font><h4></td>
	...
	else
	{
		...
		{
			print "<option value='$u[0]'>$u[2]</option>";
		}
	}
        ...
        <input type="submit" name="viewquestion" id="viewquestion" value="GO"/></td>
    </tr>
        <tr>
      <td><h4><font color="#000000">Choose Question</font><h4></td>
      <td><label for="subcat"></label>
        <select name="ques" id="subcat">
          <option>Select</option>
          <?php
	...
	$q="select * from addquestion where subcatid='$scat'";
	$res=mysqli_query($conn,$q) or die("Error in query" . mysqli_error($conn));
	$cnt=mysqli_affected_rows($conn);
	mysqli_close($conn);
	if($cnt==0)
	{
		print "<option>No Question</option>";
	}
	else
	{
		while($u=mysqli_fetch_array($res))
		{
			print "<option value='$u[0]'>$u[3]</option>";
		}
	}
        ...
        <input type="submit" name="viewanswer" id="viewquestion" value="GO"/></td>
    </tr>
  </table><br/>
   <?php
	...
	$res=mysqli_query($conn,$q) or die("Error in query" . mysqli_error($conn));
			...
			{
				print "<table width='450' cellpadding='5' align='center'>
				<tr align='left' bgcolor='#2f364a' height='35'>
					<th><h4><font color='#F4F4F4'>Answer</font></h4></th>
					<th><h4><font color='#F4F4F4'>Delete</font></h4></th>
				</tr>";
				while($x=mysqli_fetch_array($res))
				{
					print "<tr height='70'>
					<td><p>$x[2]</p></td>
					<td><a href='deleteanswer.php?acid=$x[0]'>Delete</a></td>
					</tr>";
				}
				print "</table>";
			}
