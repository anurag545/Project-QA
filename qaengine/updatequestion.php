...
<form id="form1" name="form1" method="post" action="">
  <table width="400" cellspacing="0" cellpadding="5">
   ...
   <?php
		...
		require_once("vars.php");
				$conn=mysqli_connect(host,uname,pass,dbname) or die("Error in connection" . mysqli_connect_error());
			$q="select * from addquestion where subcatid='$scid'";
	$res=mysqli_query($conn,$q) or die("Error in query" . mysqli_error($conn));
			$cnt=mysqli_affected_rows($conn);
			mysqli_close($conn);
			if($cnt==0)
			...
			else
			{
				...
				<tr align='left' bgcolor='#2f364a' height='35'>
					<th><h4><font color='#F4F4F4'>Question</font></h4></th>
					<th><h4><font color='#F4F4F4'>Update</font></h4></th>
				...
				while($x=mysqli_fetch_array($res))
				{
					print "<tr height='70'>
					<td><p>$x[3]</p></td>
		<td><a href='updatequestiondetails.php?qcid=$x[0]'>Update</a></td>
					<td><a href='deletequestion.php?qcid=$x[0]'>Delete</a></td>
					</tr>";
				}
				print "</table>";
			}
